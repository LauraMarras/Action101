...
# Create function to convert onset and offset times to decimals of seconds
...
cuts = np.arange(0, lm.shape[0]+20, 20)
cut_on = cuts[:-1]
cut_off = cuts[1:]
cut_off[-1] = lm.shape[0]

# Initialize final matrices
lm_ds_bin = np.zeros((len(cut_on), lm.shape[1]))
lm_ds_norm = np.zeros((len(cut_on), lm.shape[1]))

ai_ds_bin = np.zeros((len(cut_on), ai.shape[1]))
ai_ds_norm = np.zeros((len(cut_on), ai.shape[1]))

lt_ds_bin = np.zeros((len(cut_on), lt.shape[1]))
lt_ds_norm = np.zeros((len(cut_on), lt.shape[1]))

# Iterate over cut ons
for c, c_on in enumerate(cut_on):
    c_off = cut_off[c]

    summed_rows = lm.loc[c_on:c_off, :].sum(axis=0)    
    normalized_rows = np.divide(summed_rows, np.ones(lm.shape[1])*20)
    summed_rows[summed_rows > 0] = 1
    lm_ds_bin[c,:] = summed_rows
    lm_ds_norm[c,:] = normalized_rows

    summed_rows = ai.loc[c_on:c_off, :].sum(axis=0)    
    normalized_rows = np.divide(summed_rows, np.ones(ai.shape[1])*20)
    summed_rows[summed_rows > 0] = 1
    ai_ds_bin[c,:] = summed_rows
    ai_ds_norm[c,:] = normalized_rows

    summed_rows = lt.loc[c_on:c_off, :].sum(axis=0)    
    normalized_rows = np.divide(summed_rows, np.ones(lt.shape[1])*20)
    summed_rows[summed_rows > 0] = 1
    lt_ds_bin[c,:] = summed_rows
    lt_ds_norm[c,:] = normalized_rows
